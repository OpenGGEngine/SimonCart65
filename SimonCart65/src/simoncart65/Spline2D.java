...
import simoncart65.components.Node;
...
public class Spline2D {
    ...
    public Vector2f getPoint(double t) {
        ...
        result.x = (float) splineX.getValue(t);
        result.y = (float) splineY.getValue(t);
    ...
    public static Spline2D getFromNodes(List<Node> nodes){
        List<Vector2f> vectors = new ArrayList<>(nodes.size());
        for(Node node : nodes){
            vectors.add(new Vector2f(node.getPosition().x(), node.getPosition().z()));
        }
        vectors.add(new Vector2f(nodes.get(0).getPosition().x(), nodes.get(0).getPosition().z()));
        return new Spline2D(vectors);
    }
    
    public static Spline2D getFromNodes(Node... nodes){
        return getFromNodes(Arrays.asList(nodes));
    }
}
